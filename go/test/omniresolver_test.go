...
import (
	...
	omni "github.com/voxgig-sdk/dreamapply-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DreamapplyError); is {
				out := map[string]any{
					"name":    "DreamapplyError",
...
func omniCtx(arg any, client *sdk.DreamapplySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DreamapplySDK); is {
			client = live
