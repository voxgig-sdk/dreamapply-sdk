...
func TestApplicantDirect(t *testing.T) {
	t.Run("direct-list-applicant", func(t *testing.T) {
		setup := applicantDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-applicant", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-applicant", func(t *testing.T) {
		setup := applicantDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-applicant", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type applicantDirectSetupResult struct {
...
func applicantDirectSetup(mockres any) *applicantDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"DREAMAPPLY_TEST_APPLICANT_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["DREAMAPPLY_TEST_APPLICANT_ENTID"]; ok {
		...
		return &applicantDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &applicantDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
