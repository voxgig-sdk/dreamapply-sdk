...
func TestLoginEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Login(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Login(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set DREAMAPPLY_TEST_LOGIN_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		loginRef01Match := map[string]any{}

		loginRef01ListResult, err := loginRef01Ent.List(loginRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, loginRef01ListOk := loginRef01ListResult.([]any)
		if !loginRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", loginRef01ListResult)
		}
...
func loginBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DREAMAPPLY_TEST_LOGIN_ENTID")
