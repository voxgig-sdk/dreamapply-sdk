...
func TestFeeDirect(t *testing.T) {
	t.Run("direct-list-fee", func(t *testing.T) {
		setup := feeDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-fee", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-fee", func(t *testing.T) {
		setup := feeDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-fee", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type feeDirectSetupResult struct {
...
func feeDirectSetup(mockres any) *feeDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"DREAMAPPLY_TEST_FEE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["DREAMAPPLY_TEST_FEE_ENTID"]; ok {
		...
		return &feeDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &feeDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
