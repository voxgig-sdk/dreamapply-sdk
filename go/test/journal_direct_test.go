...
func TestJournalDirect(t *testing.T) {
	t.Run("direct-list-journal", func(t *testing.T) {
		setup := journalDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-journal", _mode); _shouldSkip {
			if _reason == "" {
...
type journalDirectSetupResult struct {
...
func journalDirectSetup(mockres any) *journalDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["DREAMAPPLY_TEST_JOURNAL_ENTID"]; ok {
		...
		return &journalDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &journalDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
