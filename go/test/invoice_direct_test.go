...
func TestInvoiceDirect(t *testing.T) {
	t.Run("direct-list-invoice", func(t *testing.T) {
		setup := invoiceDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-invoice", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-invoice", func(t *testing.T) {
		setup := invoiceDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-invoice", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type invoiceDirectSetupResult struct {
...
func invoiceDirectSetup(mockres any) *invoiceDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"DREAMAPPLY_TEST_INVOICE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["DREAMAPPLY_TEST_INVOICE_ENTID"]; ok {
		...
		return &invoiceDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &invoiceDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
