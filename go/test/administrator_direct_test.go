...
func TestAdministratorDirect(t *testing.T) {
	t.Run("direct-list-administrator", func(t *testing.T) {
		setup := administratorDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-administrator", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-administrator", func(t *testing.T) {
		setup := administratorDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-administrator", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type administratorDirectSetupResult struct {
	client *sdk.DreamapplySDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func administratorDirectSetup(mockres any) *administratorDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"DREAMAPPLY_TEST_ADMINISTRATOR_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["DREAMAPPLY_TEST_ADMINISTRATOR_ENTID"]; ok {
		...
		return &administratorDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &administratorDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
