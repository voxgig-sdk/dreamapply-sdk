...
func TestCourseDirect(t *testing.T) {
	t.Run("direct-list-course", func(t *testing.T) {
		setup := courseDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-course", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-course", func(t *testing.T) {
		setup := courseDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-course", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type courseDirectSetupResult struct {
...
func courseDirectSetup(mockres any) *courseDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"DREAMAPPLY_TEST_COURSE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["DREAMAPPLY_TEST_COURSE_ENTID"]; ok {
		...
		return &courseDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &courseDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
