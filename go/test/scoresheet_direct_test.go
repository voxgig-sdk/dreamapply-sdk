...
func TestScoresheetDirect(t *testing.T) {
	t.Run("direct-list-scoresheet", func(t *testing.T) {
		setup := scoresheetDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-scoresheet", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-scoresheet", func(t *testing.T) {
		setup := scoresheetDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-scoresheet", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type scoresheetDirectSetupResult struct {
...
func scoresheetDirectSetup(mockres any) *scoresheetDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"DREAMAPPLY_TEST_SCORESHEET_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["DREAMAPPLY_TEST_SCORESHEET_ENTID"]; ok {
		...
		return &scoresheetDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &scoresheetDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
