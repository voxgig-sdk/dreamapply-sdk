...
class TestLoginDirect:
    ...
    def test_should_direct_list_login(self):
        setup = _login_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-login", "live" if setup["live"] else "unit")
        if _skip:
...
def _login_direct_setup(mockres):
