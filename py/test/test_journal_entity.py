...
class TestJournalEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Journal(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Journal(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _journal_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "journal." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DREAMAPPLY_TEST_JOURNAL_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        journal_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.journal")))
        journal_ref01_data = None
        if len(journal_ref01_data_raw) > 0:
        ...
        journal_ref01_list_result = journal_ref01_ent.list(journal_ref01_match, None)
        assert isinstance(journal_ref01_list_result, list)



def _journal_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/journal/JournalTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("DREAMAPPLY_TEST_JOURNAL_ENTID"))
