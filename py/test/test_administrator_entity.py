...
class TestAdministratorEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Administrator(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Administrator(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Administrator(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _administrator_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "administrator." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DREAMAPPLY_TEST_ADMINISTRATOR_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        administrator_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.administrator")))
        administrator_ref01_data = None
        if len(administrator_ref01_data_raw) > 0:
            administrator_ref01_data = helpers.to_map(administrator_ref01_data_raw[0][1])

        # LIST
        administrator_ref01_ent = client.Administrator(None)
        administrator_ref01_match = {}

        administrator_ref01_list_result = administrator_ref01_ent.list(administrator_ref01_match, None)
        assert isinstance(administrator_ref01_list_result, list)

        # LOAD
        administrator_ref01_match_dt0 = {
...
def _administrator_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/administrator/AdministratorTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "DREAMAPPLY_TEST_ADMINISTRATOR_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("DREAMAPPLY_TEST_ADMINISTRATOR_ENTID"))
