...
class DreamapplyTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DreamapplyTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DreamapplyTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DreamapplyTestRunner.getenv("DREAMAPPLY_TEST_LIVE")
        override = DreamapplyTestRunner.getenv("DREAMAPPLY_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DreamapplyTestRunner.getenv(key)
        ...
        explain = DreamapplyTestRunner.getenv("DREAMAPPLY_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["DREAMAPPLY_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if DreamapplyTestRunner._test_control is not None:
            return DreamapplyTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DreamapplyTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DreamapplyTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DreamapplyTestRunner.load_test_control()
...
def load_env_local():
    DreamapplyTestRunner.load_env_local()


def env_override(m):
    return DreamapplyTestRunner.env_override(m)


def entity_data(v):
    return DreamapplyTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DreamapplyTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DreamapplyTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DreamapplyTestRunner.load_test_control()


def live_client_options():
    return DreamapplyTestRunner.live_client_options()


def live_delay_ms():
    return DreamapplyTestRunner.live_delay_ms()
