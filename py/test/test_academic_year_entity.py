...
class TestAcademicYearEntity:

    def test_should_create_instance(self):
        testsdk = DreamapplySDK.test(None, None)
        ent = testsdk.AcademicYear(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.AcademicYear(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.AcademicYear(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _academic_year_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "academic_year." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DREAMAPPLY_TEST_ACADEMIC_YEAR_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        academic_year_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.academic_year")))
        academic_year_ref01_data = None
        if len(academic_year_ref01_data_raw) > 0:
            academic_year_ref01_data = helpers.to_map(academic_year_ref01_data_raw[0][1])

        # LIST
        academic_year_ref01_ent = client.AcademicYear(None)
        academic_year_ref01_match = {}

        academic_year_ref01_list_result = academic_year_ref01_ent.list(academic_year_ref01_match, None)
...
def _academic_year_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/academic_year/AcademicYearTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "DREAMAPPLY_TEST_ACADEMIC_YEAR_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("DREAMAPPLY_TEST_ACADEMIC_YEAR_ENTID"))
