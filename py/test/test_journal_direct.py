...
class TestJournalDirect:
    ...
    def test_should_direct_list_journal(self):
        setup = _journal_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-journal", "live" if setup["live"] else "unit")
        if _skip:
...
def _journal_direct_setup(mockres):
